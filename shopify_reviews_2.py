...
from pydrive2.drive import GoogleDrive # Ensure this import is correct
...
def authenticate_drive():
    ...
    try:
        print(f"Attempting to write credentials.json to {credentials_path}")
        # Ensure the directory exists if not in root of action
        ...
        print(f"GoogleDrive instance created. Type: {type(drive_instance)}")
        return drive_instance
    except Exception as e:
        print(f"❌ Authentication failed in authenticate_drive: {e}")
...
def upload_to_drive(local_path, file_name, folder_id):
    print(f"\nAttempting to upload: {file_name} from {local_path} to folder ID: {folder_id}")
    if not os.path.exists(local_path):
        print(f"Error: Local file not found at {local_path}. Cannot upload.")
        return

    try:
        drive = authenticate_drive()
        print(f"Retrieved drive object. Type: {type(drive)}")
        print(f"Creating Google Drive file metadata for title='{file_name}', parent_id='{folder_id}'")
        file_metadata = {'title': file_name, 'parents': [{'id': folder_id}]}
        file = drive.CreateFile(file_metadata)
        print(f"File object created for {file_name}. Setting content from {local_path}...")
        ...
        # Ensure you get the full error in logs
        import traceback
